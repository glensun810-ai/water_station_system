...
def delete_booking(
    ...
    """删除预约记录（软删除，仅管理员和超级管理员可用）"""
    booking = db.query(MeetingBooking).filter(MeetingBooking.id == booking_id).first()
    if not booking:
        raise HTTPException(status_code=404, detail="预约不存在")

    if booking.is_deleted == 1:
        raise HTTPException(status_code=400, detail="该预约已被删除")

    booking.is_deleted = 1
    booking.deleted_at = datetime.now()
    booking.deleted_by = deleted_by or "管理员"
    booking.delete_reason = delete_reason or "管理员删除"

    db.commit()
    return {"message": "预约记录已删除", "booking_id": booking_id}
