...
def calculate_coupon_discount(
    coupon: Coupon, order_amount: float, payment_method: str
) -> float:
    """
    计算优惠券折扣金额
...
def create_coupon(
    ...
    if request.type not in ["discount", "fixed"]:
        raise HTTPException(
            status_code=400, detail="优惠券类型必须是 'discount' 或 'fixed'"
        )
    ...
    if request.applicable_modes:
        for mode in request.applicable_modes:
            if mode not in ["prepaid", "credit"]:
                raise HTTPException(
                    status_code=400, detail="适用模式只能是 'prepaid' 或 'credit'"
                )
    ...
    coupon = Coupon(
        ...
        applicable_products=json.dumps(request.applicable_products)
        if request.applicable_products
        else None,
        applicable_modes=json.dumps(request.applicable_modes)
        if request.applicable_modes
        else None,
    ...
    return {"message": "优惠券创建成功", "coupon": coupon.to_dict()}
...
def issue_coupon_to_users(
    ...
    if coupon.total_quantity > 0:
        ...
        if len(request.user_ids) > remaining:
            raise HTTPException(
                status_code=400, detail=f"优惠券剩余数量不足，仅剩{remaining}张"
            )
...
def get_my_coupons(
    ...
    user_coupons = query.order_by(UserCoupon.expires_at.asc()).all()
    ...
    return {"coupons": results, "total": len(results)}
...
def calculate_best_coupon(
    ...
    user_coupons = (
        db.query(UserCoupon)
        .filter(UserCoupon.user_id == current_user.id, UserCoupon.status == "unused")
        .all()
    )
    ...
    for uc in user_coupons:
        ...
        discount = calculate_coupon_discount(
            coupon, request.order_amount, request.payment_method
        )
...
def use_coupon(
    ...
    order = (
        ...
        .filter(
            UnifiedOrder.id == request.order_id, UnifiedOrder.user_id == current_user.id
        )
        .first()
    )
    ...
    # 计算折扣金额
    discount = calculate_coupon_discount(
        coupon, order.total_amount, order.payment_method
    )
