...
def get_prepaid_wallet(db: Session, user_id: int, product_id: int) -> Optional[AccountWallet]:
    """获取用户预付钱包"""
    wallet = db.query(AccountWallet).filter(
...
def get_promotion_config(db: Session, product_id: int, promotion_type: str) -> Optional[PromotionConfigV2]:
    """获取产品的优惠配置"""
...
def smart_payment_recommend(
    ...
    return {
        ...
        'promo_config': {
            'trigger_qty': promo_config.trigger_qty if promo_config else 0,
            'gift_qty': promo_config.gift_qty if promo_config else 0,
            'rule': f'买{promo_config.trigger_qty}赠{promo_config.gift_qty}' if promo_config else '无优惠'
        }
    }
...
def create_pickup_order(
    ...
    payment_method = request.preferred_payment or recommendation.get('recommended_method')
...
def pay_order(
    ...
    transaction = UnifiedTransaction(
        ...
        payment_details=json.dumps({
            'credit_qty': 0 if order.payment_method == 'prepaid' else order.quantity,
...
def process_prepaid_payment(db: Session, order: UnifiedOrder, user_id: int):
    ...
    if wallet.paid_qty < required_paid_qty:
        raise HTTPException(
            400, 
            f"付费桶余额不足，需要{required_paid_qty}，可用{wallet.paid_qty}"
        )
    
    if wallet.free_qty < required_gift_qty:
        raise HTTPException(
            400, 
            f"赠送桶余额不足，需要{required_gift_qty}，可用{wallet.free_qty}"
        )
...
def get_orders(
    ...
    - 按时间范围
    """
    query = db.query(UnifiedOrder).filter(
        UnifiedOrder.user_id == current_user.id
    )
...
def get_transactions(
    ...
    transactions = query.order_by(
        UnifiedTransaction.created_at.desc()
    ).offset(skip).limit(limit).all()
