...
class Coupon(Base):
    ...
    # 适用范围
    applicable_products = Column(
        String, nullable=True
    )  # JSON 格式存储适用产品 ID 列表，NULL 表示通用
    applicable_modes = Column(
        String, nullable=True
    )  # JSON 格式存储适用模式 ['prepaid', 'credit']，NULL 表示通用
    ...
    def to_dict(self):
        ...
        return {
            ...
            "applicable_modes": json.loads(self.applicable_modes)
            if self.applicable_modes
            else None,
            "valid_from": self.valid_from.strftime("%Y-%m-%d %H:%M:%S")
            if self.valid_from
            else None,
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and (self.total_quantity == 0 or self.issued_quantity < self.total_quantity)
        )
...
class UserCoupon(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "coupon_code": self.coupon_code,
            "status": self.status,
            "used_at": self.used_at.strftime("%Y-%m-%d %H:%M:%S")
            if self.used_at
            else None,
