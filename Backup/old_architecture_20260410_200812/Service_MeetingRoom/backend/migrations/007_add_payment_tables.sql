-- 会议管理支付结算功能增强（修订版）
-- 创建时间: 2026-04-02
-- 说明: meeting_bookings表的支付相关字段已存在，只需创建新表和索引
...
-- 7. 创建索引优化查询（部分索引可能已存在，使用IF NOT EXISTS）
CREATE INDEX IF NOT EXISTS idx_booking_payment_status ON meeting_bookings(payment_status);
CREATE INDEX IF NOT EXISTS idx_booking_payment_mode ON meeting_bookings(payment_mode);
CREATE INDEX IF NOT EXISTS idx_booking_date_range ON meeting_bookings(booking_date, start_time, end_time);
...
CREATE INDEX IF NOT EXISTS idx_settlement_period ON meeting_settlement_batches(settlement_period_start, settlement_period_end);
...
CREATE INDEX IF NOT EXISTS idx_payment_office ON meeting_payment_records(office_id);
CREATE INDEX IF NOT EXISTS idx_free_quota_period ON meeting_free_quota(year_month);
CREATE INDEX IF NOT EXISTS idx_stats_date ON meeting_room_statistics(stat_date);
CREATE INDEX IF NOT EXISTS idx_operation_type ON meeting_operation_logs(operation_type);
CREATE INDEX IF NOT EXISTS idx_operation_time ON meeting_operation_logs(created_at);