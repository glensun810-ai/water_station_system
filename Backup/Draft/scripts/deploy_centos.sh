...
elif [ -d "root/project/Service_WaterManage/backend" ]; then
    log_info "从/root/project复制后端代码..."
    cp -r /root/project/Service_WaterManage/backend/* "${DEPLOY_DIR}/backend/"
elif [ -d "/root/Service_WaterManage/backend" ]; then
    log_info "从/root复制后端代码..."
    cp -r /root/Service_WaterManage/backend/* "${DEPLOY_DIR}/backend/"
else
    log_error "未找到后端代码目录"
    log_info "请检查以下位置是否存在代码："
    log_info "  - $(pwd)/Service_WaterManage/backend"
    log_info "  - /root/project/Service_WaterManage/backend"
    log_info "  - /root/Service_WaterManage/backend"
    exit 1
fi
...
elif [ -d "/root/project/Service_WaterManage/frontend" ]; then
...
elif [ -d "/root/project/Service_MeetingRoom/frontend" ]; then
