...
@router.get("/api/membership/plans", response_model=MembershipPlanList)
...
@router.get("/api/membership/plans/{plan_id}", response_model=MembershipPlanResponse)
async def get_membership_plan(plan_id: int, db: Session = Depends(get_db)):
    """
    获取单个会员套餐详情
    """
    try:
