...
class Office(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(100), nullable=False)  # 办公室名称
    room_number = Column(String(50), nullable=True)  # 房间号
    description = Column(String(500), nullable=True)  # 描述

    # 负责人信息
    leader_name = Column(String(100), nullable=True)  # 负责人姓名

    # 配置人数
    water_user_count = Column(Integer, default=0)  # 用水人数/配置人数

    # 常用标记
    is_common = Column(Integer, default=1)  # 1: 常用，0: 不常用

    # 超级管理员关联
    super_admin_id = Column(Integer, nullable=True)  # 关联的管理员用户ID
