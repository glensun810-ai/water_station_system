...
def create_reservation(
    ...
    if product.stock < request.pickup_qty:
    ...
    # 冻结库存
    product.stock -= request.pickup_qty
    ...
    reservation = Transaction(
        ...
        reserved_qty=request.pickup_qty,
        used_qty=0,
        actual_price=actual_price,
        type="reservation",
        mode="prepay",
...
@app.post("/api/reservation/pickup")
def pickup_reservation(
    ...
    if reservation.status != "reserved":
        raise HTTPException(status_code=400, detail="预定记录状态异常")

    # 计算剩余可领取数量
    remaining = reservation.reserved_qty - reservation.used_qty
    if remaining <= 0:
        raise HTTPException(status_code=400, detail="无可领取数量")

    if request.pickup_qty <= 0:
        raise HTTPException(status_code=400, detail="领取数量必须大于 0")
